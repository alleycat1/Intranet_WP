...
class PluginUpdate_2_2_custom {
	...
	public function toWpFormat(){
		...
		if( $this->slug == 'dpProEventCalendar' )
			$purchase_code = pec_setting( 'purchase_code' );

		if( $this->slug == 'dp-pec-payments' )
			$purchase_code = pec_payments_setting( 'purchase_code' );
		...
		$domain = str_replace(".", "-", urlencode($domain));

		$update->package = $this->download_url.'?purchaseCode=' . $purchase_code . '&domain='.$domain . '&plugin='.$this->slug;
