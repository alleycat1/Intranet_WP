...
if ( current_user_can( 'administrator' ) ) {
	$tickets = wp_yap_get_list_closed_ticket();
} else {
	$tickets = wp_yap_get_list_closed_ticket_posht( get_current_user_id() );
}
?>
...
<table class="wp-list-table widefat striped">
	...
	<?php foreach ( $tickets[0] as $ticket ) : ?>
		<tr style="border: solid 1px #ccc">
			...
			<th><a href="<?php echo get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=nirweb_ticket_send_ticket&action=edit&id=' . esc_html( $ticket->ticket_id ); ?>" class="su_link_tik"><?php echo esc_html( $ticket->subject ); ?></a></th>
			...
			<th><a href="<?php echo get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=nirweb_ticket_send_ticket&action=edit&id=' . esc_html( $ticket->ticket_id ); ?>" class="edit_ticket_wpys"><span class="dashicons dashicons-edit"></span></a></a></th>
		</tr>
	<?php endforeach; ?>
	</tbody>
</table>
</form>
<div class="nirweb_ticket_pagination"> <?php echo wp_unslash( $tickets[1] ); ?> </div>
