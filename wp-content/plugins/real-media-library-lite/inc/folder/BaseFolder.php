...
abstract class BaseFolder implements IFolder
{
    ...
    public function anyParentHas($column, $value = null, $valueFormat = '%s', $includeSelf = \false, $until = null)
    {
        ...
        $sql = \wp_rml_create_all_parents_sql($this, $includeSelf, $until, ['fields' => "rmldata.id, rmldata.{$column}", 'afterWhere' => $wpdb->prepare("AND rmldata.{$column} = {$valueFormat}", $value)]);
    ...
    public function anyParentHasMetadata($meta_key, $meta_value = null, $valueFormat = '%s', $includeSelf = \false, $until = null)
    {
        global $wpdb;
        // phpcs:disable WordPress.DB
        $sql = \wp_rml_create_all_parents_sql($this, $includeSelf, $until, ['fields' => 'rmlmeta.meta_id AS id, rmlmeta.realmedialibrary_id AS folderId, rmlmeta.meta_value AS value', 'join' => $wpdb->prepare('JOIN ' . $this->getTableName('meta') . ' rmlmeta ON rmlmeta.realmedialibrary_id = rmldata.id AND rmlmeta.meta_key = %s', $meta_key), 'afterWhere' => $meta_value === null ? "AND TRIM(IFNULL(rmlmeta.meta_value,'')) <> ''" : $wpdb->prepare("AND rmlmeta.meta_value = {$valueFormat}", $meta_value)]);
        return $sql === \false ? [] : $wpdb->get_results($sql, ARRAY_A);
        // phpcs:enable WordPress.DB
    }
    // Documented in IFolder
    public function anyChildrenHas($column, $value = null, $valueFormat = '%s', $includeSelf = \false)
    {
        global $wpdb;
        // phpcs:disable WordPress.DB
        $sql = \wp_rml_create_all_children_sql($this, $includeSelf, ['fields' => "rmldata.id, rmldata.{$column}", 'afterWhere' => $wpdb->prepare("AND rmldata.{$column} = {$valueFormat}", $value)]);
        return $sql === \false ? [] : $wpdb->get_results($sql, ARRAY_A);
        // phpcs:enable WordPress.DB
    }
    // Documented in IFolder
    ...
    public function getCnt($forceReload = \false)
    {
        if ($this->cnt === null || $forceReload) {
            $query = new \MatthiasWeb\RealMediaLibrary\folder\QueryCount(
                 ...
                 */
                \apply_filters('RML/Folder/QueryCountArgs', ['post_status' => 'inherit', 'post_type' => 'attachment', 'rml_folder' => $this->id])
            );
    ...
    public function getPlain($deep = \false)
    {
        ...
        $result = ['id' => $this->getId(), 'type' => $this->getType(), 'parent' => $this->getParent(), 'name' => $this->getName(), 'order' => $this->getOrder(), 'restrictions' => $this->getRestrictions(), 'slug' => $this->getSlug(), 'absolutePath' => $this->getAbsolutePath(), 'cnt' => $this->getCnt(), 'contentCustomOrder' => (int) $this->getContentCustomOrder(), 'forceCustomOrder' => $this->forceContentCustomOrder(), 'lastOrderBy' => $this->getRowData('lastOrderBy'), 'orderAutomatically' => $this->getRowData('orderAutomatically'), 'lastSubOrderBy' => $this->getRowData('lastSubOrderBy'), 'subOrderAutomatically' => $this->getRowData('subOrderAutomatically')];
        // Add the children
        if ($deep) {
            $children = [];
    ...
    public function isRestrictFor($restriction)
    {
        ...
        return \in_array($restriction, $this->restrictions, \true) || \in_array($restriction . '>', $this->restrictions, \true);
    }
}
