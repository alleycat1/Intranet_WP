...
namespace Barn2\Plugin\Document_Library_Pro\Dependencies\enshrined\svgSanitize\ElementReference;
...
class Subject
{
    ...
    public function hasInfiniteLoop(array $subjects = [], $level = 1)
    {
        if ($level > $this->useNestingLimit) {
            throw new \Barn2\Plugin\Document_Library_Pro\Dependencies\enshrined\svgSanitize\Exceptions\NestingException('Nesting level too high, aborting', 1570713498, null, $this->getElement());
        }
