<div class="front_page_section front_page_section_features<?php
	$pubzinne_scheme = pubzinne_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $pubzinne_scheme ) && ! pubzinne_is_inherit( $pubzinne_scheme ) ) {
		echo ' scheme_' . esc_attr( $pubzinne_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( pubzinne_get_theme_option( 'front_page_features_paddings' ) );
	if ( pubzinne_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$pubzinne_css      = '';
		$pubzinne_bg_image = pubzinne_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$pubzinne_anchor_icon = pubzinne_get_theme_option( 'front_page_features_anchor_icon' );
	$pubzinne_anchor_text = pubzinne_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( pubzinne_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$pubzinne_css      = '';
			$pubzinne_bg_mask  = pubzinne_get_theme_option( 'front_page_features_bg_mask' );
			$pubzinne_bg_color_type = pubzinne_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $pubzinne_bg_color_type ) {
				$pubzinne_bg_color = pubzinne_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$pubzinne_caption = pubzinne_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $pubzinne_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $pubzinne_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $pubzinne_caption, 'pubzinne_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$pubzinne_description = pubzinne_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $pubzinne_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $pubzinne_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $pubzinne_description ), 'pubzinne_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! pubzinne_exists_trx_addons() ) {
						...
						pubzinne_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
